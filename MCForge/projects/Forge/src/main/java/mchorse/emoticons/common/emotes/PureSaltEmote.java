...
{
    public PureSaltEmote(String name, int duration, boolean looping, ResourceLocation sound, ResourceLocation location)
    {
    ...
    public void progressAnimation(EntityLivingBase entity, BOBJArmature armature, AnimatorEmoticonsController animator, int tick, float partial)
    {
        if (tick > 18 && tick <= 78 && tick % 2 == 0)
        {
            ...
            {
                SaltParticle salt = new SaltParticle(entity.worldObj, result.x, result.y, result.z, 0);
