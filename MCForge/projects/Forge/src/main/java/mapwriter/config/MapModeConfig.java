...
public class MapModeConfig {
	...
	public static final String[] coordsModeStringArray = {
			...
			"mw.config.map.coordsMode.small", "mw.config.map.coordsMode.large" };

	public static final String[] miniMapPositionStringArray = {
			"mw.config.map.position.topRight",
			"mw.config.map.position.topLeft",
			"mw.config.map.position.botRight", "mw.config.map.position.botLeft" };
	...
	public void loadConfig() {
		...
		this.playerArrowSize = ConfigurationHandler.configuration.getInt(
				"playerArrowSize", this.configCategory,
				this.playerArrowSizeDef, 1, 20, "",
				"mw.config.map.playerArrowSize");
		this.markerSize = ConfigurationHandler.configuration.getInt(
				"markerSize", this.configCategory, this.markerSizeDef, 1, 20,
				"", "mw.config.map.markerSize");
		this.alphaPercent = ConfigurationHandler.configuration.getInt(
				"alphaPercent", this.configCategory, this.alphaPercentDef, 0,
				100, "", "mw.config.map.alphaPercent");
	...
	public IConfigElement categoryElement(String name, String tooltip_key) {
		return new DummyCategoryElement(name, tooltip_key, new ConfigElement(
				ConfigurationHandler.configuration
						.getCategory(this.configCategory)).getChildElements());
	}
}