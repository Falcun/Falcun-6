...
import mapwriter.map.MarkerManagerGroup;
...
public class Mw {
	...
	public FileSaving facSaving = new FileSaving();
	...
	public MarkerManager markerManager = new MarkerManager();
	public MarkerManagerGroup markerManagerGroup = new MarkerManagerGroup();
	...
	public List<Long> groupEdited = new CopyOnWriteArrayList<>();
	...
	public void setTextureSize() {
		if (Config.configTextureSize != this.textureSize) {
			...
			while ((textureSize <= maxTextureSize)
					&& (textureSize <= Config.configTextureSize)) {
				textureSize *= 2;
			}
			textureSize /= 2;

			Logging.log("GL reported max texture size = %d", maxTextureSize);
			Logging.log("texture size from config = %d",
					Config.configTextureSize);
	...
	public void updatePlayer() {
		...
		if (this.mc.theWorld != null) {
			if (!this.mc.theWorld.getChunkFromBlockCoords(
					new BlockPos(this.playerX, 0, this.playerZ)).isEmpty()) {
				this.playerBiome = this.mc.theWorld
						.getBiomeGenForCoords(new BlockPos(this.playerX, 0,
								this.playerZ)).biomeName;
			}
		}
	...
	public void toggleMarkerMode() {
		...
		this.mc.thePlayer.addChatMessage(new ChatComponentText("group "
				+ this.markerManager.getVisibleGroupName() + " selected"));
		this.markerManagerGroup.nextGroup();
		this.markerManagerGroup.update();
		this.mc.thePlayer.addChatMessage(new ChatComponentText("group "
				+ this.markerManagerGroup.getVisibleGroupName() + " selected"));
	}

	// cheap and lazy way to teleport...
	public void teleportTo(int x, int y, int z) {
		if (Config.teleportEnabled) {
			this.mc.thePlayer.sendChatMessage(String.format("/%s %d %d %d",
					Config.teleportCommand, x, y, z));
		} else {
	...
	public void loadBlockColourOverrides(BlockColours bc) {
		File f = new File(this.configDir,
				Reference.blockColourOverridesFileName);
		if (f.isFile()) {
			...
			if (f.isFile()) {
				...
				Logging.logError(
						"could not load block colour overrides from file %s", f);
			}
		}
	}
	...
	public void reloadMapTexture() {
		...
		MapTexture newMapTexture = new MapTexture(this.textureSize,
		...
		this.regionManager = new RegionManager(this.worldDir, this.imageDir,
				this.blockColours, Config.zoomInLevels, Config.zoomOutLevels);
	}
	...
	public void load() {
		...
		if (Config.saveDirOverride.length() > 0) {
			...
			if (d.isDirectory()) {
				...
				Logging.log("error: no such directory %s",
						Config.saveDirOverride);
			}
		}

		if (!this.mc.isSingleplayer()) {
			this.worldDir = new File(new File(saveDir, "mapwriter_mp_worlds"),
					Utils.getWorldName());
		} else {
			this.worldDir = new File(new File(saveDir, "mapwriter_sp_worlds"),
					Utils.getWorldName());
		}
		...
		this.markerManager.load(WorldConfig.getInstance().worldConfiguration,
				Reference.catMarkers);

		this.markerManagerGroup = new MarkerManagerGroup();
		this.markerManagerGroup.load(WorldConfig.getInstance().worldConfiguration,
				Reference.catMarkers);
	...
	public void close() {
		...
		if (this.ready) {
			...
			this.executor
					.addTask(new CloseRegionManagerTask(this.regionManager));
			this.regionManager = null;

			Logging.log("waiting for %d tasks to finish...",
					this.executor.tasksRemaining());
			...
			this.markerManagerGroup.save(
					WorldConfig.getInstance().worldConfiguration,
					Reference.catMarkers);
			this.markerManagerGroup.clear();
	...
	public void onTick() {
		...
		if (this.ready && (this.mc.thePlayer != null)) {
			...
			if (!(this.mc.currentScreen instanceof MwGui)) {
				...
				// and render it.
				this.miniMap.view.setViewCentreScaled(this.playerX,
						this.playerZ, this.playerDimension);
	...
	public void onChunkLoad(Chunk chunk) {
		...
		this.load();
		if ((chunk != null)
				&& (chunk.getWorld() instanceof net.minecraft.client.multiplayer.WorldClient)) {
			if (this.ready) {
				this.chunkManager.addChunk(chunk);
			} else {
				Logging.logInfo("missed chunk (%d, %d)", chunk.xPosition,
						chunk.zPosition);
			}
		}
	}
	...
	public void onPlayerDeath(EntityPlayerMP player) {
		...
		if (this.ready && (Config.maxDeathMarkers > 0)) {
			this.updatePlayer();
			int deleteCount = (this.markerManager
					.countMarkersInGroup("playerDeaths") - Config.maxDeathMarkers) + 1;
			...
			this.markerManager.addMarker(Utils.getCurrentDateString(),
					"playerDeaths", this.playerXInt, this.playerYInt,
					this.playerZInt, this.playerDimension, 0xffff0000, this.playerServer);
	...
	public void onKeyDown(KeyBinding kb) {
		...
		if ((this.mc.currentScreen == null) && (this.ready)) {
			...
			if (kb == MwKeyHandler.keyMapMode) {
				...
				String group1 = this.markerManagerGroup.getVisibleGroupName();
				if (group1.equals("none")) {
					group1 = "group";
				}
				if (Config.newMarkerDialog) {
					this.mc.displayGuiScreen(new MwGuiMarkerDialogNew(null,
							this.markerManager, "", group, this.playerXInt,
							this.playerYInt, this.playerZInt,
							this.playerDimension, this.playerServer));
				} else {
					this.mc.displayGuiScreen(new MwGuiMarkerDialog(null,
							this.markerManager, "", group, this.playerXInt,
							this.playerYInt, this.playerZInt,
							this.playerDimension, this.playerServer));
				}
				...
				this.markerManagerGroup.update();
				this.mc.thePlayer.addChatMessage(new ChatComponentText("group "
						+ this.markerManagerGroup.getVisibleGroupName()
						+ " selected"));
