...
public class MergeToImage {
	...
	public static BufferedImage mergeRegions(RegionManager regionManager,
			int x, int z, int w, int h, int dimension) {
		// create the image and graphics context
		// this is the most likely place to run out of memory
		BufferedImage mergedImage = new BufferedImage(w, h,
		...
		for (int zi = 0; zi < h; zi += Region.SIZE) {
			for (int xi = 0; xi < w; xi += Region.SIZE) {
				...
				// get region pixels
				Region region = regionManager.getRegion(x + xi, z + zi, 0,
						dimension);
				int[] regionPixels = region.surfacePixels.getPixels();
				if (regionPixels != null) {
					mergedImage.setRGB(xi, zi, Region.SIZE, Region.SIZE,
							regionPixels, 0, Region.SIZE);
				}
			}
		}
	...
	public static int merge(RegionManager regionManager, int xCentre,
			int zCentre, int w, int h, int dimension, File dir, String basename) {
		...
		Logging.logInfo("merging area starting at (%d,%d), %dx%d blocks", xMin,
				zMin, w, h);
		...
		for (int z = zMin; z < zMax; z += MAX_WIDTH) {
			...
			for (int x = xMin; x < xMax; x += MAX_WIDTH) {
				...
				String imgName = String.format("%s.%d.%d.png", basename,
						countX, countZ);
				File f = new File(dir, imgName);
				Logging.logInfo("merging regions to image %s", f);

				BufferedImage img = mergeRegions(regionManager, x, z, imgW,
						imgH, dimension);
				writeImage(img, f);
