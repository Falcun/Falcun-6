...
public class SurfacePixels {
	...
	public void updateChunk(MwChunk chunk) {
		...
		// more directly
		ChunkRender.renderSurface(this.region.regionManager.blockColours,
				chunk, pixels, offset, Region.SIZE, (chunk.dimension == -1) // use
				// ceiling
				// algorithm
				// for
				// nether
	...
	public void updateScaled(int[] srcPixels, int srcX, int srcZ, int dstX,
			int dstZ, int dstW, int dstH) {
		int[] dstPixels = this.getOrAllocatePixels();
		for (int j = 0; j < dstH; j++) {
			for (int i = 0; i < dstW; i++) {
				int srcOffset = ((srcZ + (j * 2)) << Region.SHIFT)
						+ (srcX + (i * 2));
				int dstPixel = getAverageOfPixelQuad(srcPixels, srcOffset,
						Region.SIZE);
	...
	public static void saveImage(File filename, int[] pixels, int w, int h) {
		...
		try {
			...
			Logging.logError("saveImage: error: could not write image to %s",
					filename);
		}
	}
