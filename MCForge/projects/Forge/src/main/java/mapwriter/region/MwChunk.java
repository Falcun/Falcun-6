...
public class MwChunk implements IChunk {
	...
	public MwChunk(int x, int z, int dimension, char[][] data,
			byte[][] lightingArray, byte[] biomeArray,
	...
	public static MwChunk read(int x, int z, int dimension,
		...
		if (dis != null) {
			try {
				...
				for (int k = 0; k < sections.tagCount(); ++k) {
					...
					NibbleArray nibblearray1 = section.hasKey("Add", 7) ? new NibbleArray(
							section.getByteArray("Add")) : null;
					...
					for (int l = 0; l < data[y & 0xf].length; ++l) {
						...
						int j1 = (l >> 8) & 15;
						int k1 = (l >> 4) & 15;
						int l1 = nibblearray1 != null ? nibblearray1.get(i1,
								j1, k1) : 0;
						data[y & 0xf][l] = (char) ((l1 << 12)
								| ((lsbArray[y][l] & 255) << 4) | nibblearray
								.get(i1, j1, k1));
					}
				}
		...
		return new MwChunk(x, z, dimension, data, lightingArray, biomeArray,
				TileEntityMap);
	}
	...
	public int getBiome(int x, int z) {
		return (this.biomeArray != null) ? (this.biomeArray[((z & 0xf) << 4)
				| (x & 0xf)]) & 0xff : 0;
	}
	...
	public int getBlockAndMetadata(int x, int y, int z) {
		...
		char data = ((this.dataArray != null) && (this.dataArray[yi] != null) && (this.dataArray[yi].length != 0)) ? this.dataArray[yi][offset]
				: 0;
