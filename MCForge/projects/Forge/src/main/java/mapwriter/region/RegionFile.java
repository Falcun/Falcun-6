...
public class RegionFile {
	...
	private void setFilledSectorArray(Section section, boolean filled) {
		...
		for (int i = section.startSector; i < endSector; i++) {
			if (filled && this.filledSectorArray.get(i)) {
				Logging.logError(
						"sector %d already filled, possible chunk overlap", i);
			}
	...
	private boolean checkSectionOverlaps(Section section) {
		...
		int endSector = Math.min(section.startSector + section.length,
				this.filledSectorArray.size());
	...
	public void printInfo() {
		...
		Logging.logInfo(
				"Region File %s: filled sectors = %d, free sectors = %d", this,
				filledCount, freeCount);
	...
	private void updateChunkSection(int x, int z, Section newSection)
			throws IOException {
	...
	public boolean open() {
		...
		try {
				...
				for (int i = 0; i < 1024; i++) {
					...
					if (section.length > 0) {
						...
						if (!this.checkSectionOverlaps(section)) {
							...
							Logging.logError(
									"chunk %d overlaps another chunk, file may be corrupt",
									i);
						}
					}
				}
			...
			this.fin = null;
			Logging.logError("exception when opening region file '%s': %s",
					this.file, e);
			...
			if ((section != null) && (section.length > 0)) {
				...
				try {
					...
					if ((length > 1)
						...
						Logging.logError(
								"data length (%d) or version (%d) invalid for chunk (%d, %d)",
								length, version, x, z);
					}
				} catch (Exception e) {
					Logging.logError(
							"exception while reading chunk (%d, %d): %s", x, z,
							e);
					dis = null;
				}
			}
	...
	private class RegionFileChunkBuffer extends ByteArrayOutputStream {
		...
		@Override
		public void close() {
			this.regionFile.writeCompressedChunk(this.x, this.z, this.buf,
					this.count);
		}
	}
	...
	public DataOutputStream getChunkDataOutputStream(int x, int z) {
		return new DataOutputStream(new DeflaterOutputStream(
				new RegionFileChunkBuffer(this, x, z)));
	}
	...
	private void writeChunkDataToSection(Section section,
			byte[] compressedChunkData, int length) throws IOException {
	...
	private boolean writeCompressedChunk(int x, int z,
			byte[] compressedChunkData, int length) {
		...
		if (length <= 0) {
			Logging.logWarning("not writing chunk (%d, %d) with length %d", x,
					z, length);
			return true;
		}
		...
		if ((currentSection != null)
				&& (requiredSectors <= currentSection.length)) {
		...
		try {
			...
			this.writeChunkDataToSection(newSection, compressedChunkData,
					length);
			// update the header
			...
			Logging.logError(
					"could not write chunk (%d, %d) to region file: %s", x, z,
					e);
		}
