...
public class MwGuiMarkerListOverlay extends MwGuiSlot {
	...
	public MwGuiMarkerListOverlay(GuiScreen parentScreen,
	...
	protected int getSlotHeight(int index) {
		...
		int height = this.mc.fontRendererObj.splitStringWidth(str,
				MwGuiMarkerListOverlay.listWidth - 6);
	...
	protected void elementClicked(int slotIndex, boolean isDoubleClick,
		...
		this.markerManager.selectedMarker = this.markerManager.visibleMarkerList
				.get(slotIndex);
		if (mouseButton == 1) {
			if (this.parentScreen instanceof MwGui) {
				((MwGui) this.parentScreen).openMarkerGui(
						this.markerManager.selectedMarker, mouseX, mouseY);
				;
			}
		}
	...
	protected boolean isSelected(int slotIndex) {
		return this.markerManager.selectedMarker == this.markerManager.visibleMarkerList
				.get(slotIndex);
	}
	...
	@Override
	protected void drawSlot(int entryID, int x, int y, int slotHeight,
			int mouseXIn, int mouseYIn) {
		MwGuiLabel label = new MwGuiLabel(this.getLabelString(entryID), null,
				x, y, false, false, MwGuiMarkerListOverlay.listWidth,
				this.height);
