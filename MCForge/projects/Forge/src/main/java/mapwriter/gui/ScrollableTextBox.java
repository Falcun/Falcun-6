...
public class ScrollableTextBox extends ScrollableField {
	...
	ScrollableTextBox(int x, int y, int width, String label,
			List<String> scrollableElements, FontRenderer fontrendererObj) {
		super(x, y, width, label, fontrendererObj);
	...
	private void init() {
		...
		this.textFieldWidth = this.width - 5
				- (ScrollableField.arrowsWidth * 2);

		this.textField = new GuiTextField(0, this.fontrendererObj,
				this.textFieldX, this.textFieldY, this.textFieldWidth,
...
class ScrollableNumericTextBox extends ScrollableTextBox {
	...
	public void setText(int num) {
		if ((this.maxValue < 0) || (num <= this.maxValue)
				|| (num >= this.minValue)) {
	...
	public void KeyTyped(char c, int key) {
		if (((c >= '0') && (c <= '9')) || (key == Keyboard.KEY_BACK)
			...
			if (Character.isDigit(c)
					&& ((this.maxValue > -1) && (Integer.parseInt(this
							.getText() + c) > this.maxValue))) {
				return;
			}
