...
public class GuiSchematicMaterials extends GuiScreenBase {
	...
	public void initGui() {
		...
		this.btnDump = new GuiButton(++id, this.width / 2 - 50, this.height - 30, 100, 20, toDisplay);
				...
				if(this.blockList.size() > 0) {
					if(!this.isSelectAll) {
						...
						for(int i = 0; i < this.blockList.size(); i++) {
							WrappedItemStack wis = this.blockList.get(i);
							if(wis != null) {
						...
						Wrapper.getInstance().addChat("Added tracers for " + net.mattbenson.modules.types.mods.Schematica.missinglocation.size() + " blocks");
						toDisplay = "Unselect all";
					} else {
						net.mattbenson.modules.types.mods.Schematica.missinglocation.clear();
						Wrapper.getInstance().addChat("Cleared all tracers!");
						toDisplay = "Trace all";
					}
					this.btnDump.displayString = toDisplay;
