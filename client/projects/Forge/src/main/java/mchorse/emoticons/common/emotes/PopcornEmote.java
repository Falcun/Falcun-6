...
{
    public PopcornEmote(String name, int duration, boolean looping, ResourceLocation sound, ResourceLocation location)
    {
    ...
    public void progressAnimation(EntityLivingBase entity, BOBJArmature armature, AnimatorEmoticonsController animator, int tick, float partial)
    {
        if (tick == 8 || tick == 32 || tick == 56 || tick == 86)
        {
            ...
            {
                PopcornParticle salt = new PopcornParticle(entity.worldObj, result.x, result.y, result.z, 0.1);
