...
{
    public CryingEmote(String name, int duration, boolean looping, ResourceLocation sound, ResourceLocation location)
    {
    ...
    public void progressAnimation(EntityLivingBase entity, BOBJArmature armature, AnimatorEmoticonsController animator, int tick, float partial)
    {
        if (tick % 2 == 0)
        {
            ...
            entity.worldObj.spawnParticle(EnumParticleTypes.WATER_DROP, result.x, result.y, result.z, 1, -1, 1);
        }
    }
}