...
public abstract class World implements IBlockAccess {
	...
	public final Long2ObjectMap<ExplosionCache> explosionCache = new Long2ObjectOpenHashMap<>(256, 0.9F);
...
//		return this.chunkProvider.provideChunk(chunkX, chunkZ);
	...
	public boolean spawnEntityInWorld(Entity entityIn) {
		...
		if (!flag && !this.isChunkLoaded(i, j, true)) {
			...
			if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.entity.EntityJoinWorldEvent(entityIn, this)) && !flag)
				return false;
...
//		explosion.doExplosionA();
			...
			for (float f = 0.0F; f <= 1.0F; f = (float) ((double) f + d0)) {
				for (float f1 = 0.0F; f1 <= 1.0F; f1 = (float) ((double) f1 + d1)) {
					for (float f2 = 0.0F; f2 <= 1.0F; f2 = (float) ((double) f2 + d2)) {
						...
						double d7 = axisalignedbb.minY + (axisalignedbb.maxZ - axisalignedbb.minZ) * (double) f2;
						if (this.rayTraceBlocks(new Vec3(d5 + d3, d6, d7 + d4), vec3d) == null) {
							++i;
						}
	...
	public Entity[] getEntities(AxisAlignedBB axisalignedbb) {
		Entity[] array = new Entity[0];
		int i = MathHelper.floor((axisalignedbb.minX - 2.0D) / 16.0D);
		int j = MathHelper.floor((axisalignedbb.maxX + 2.0D) / 16.0D);
		int t = MathHelper.floor((axisalignedbb.minY - 2.0D) / 16.0D);
		int f = MathHelper.floor((axisalignedbb.maxY + 2.0D) / 16.0D);
		...
		for (int i1 = i; i1 <= j; ++i1) {
			for (int j1 = k; j1 <= l; ++j1) {
				...
				if (chunk != null) {
					for (int t1 = t; t1 <= f; ++t1) {
						if (t1 >= 0 && t1 <= 15) {
							ClassInheritanceMultiMap<Entity> slice = chunk.entityLists[t1];
							if (slice != null && !slice.isEmpty()) {
								array = (Entity[]) ArrayUtils.addAll(array, slice.toArray());
							}
						}
					}
				}
