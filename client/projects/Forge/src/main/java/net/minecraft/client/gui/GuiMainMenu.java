...
import net.mattbenson.animated.AnimatedLogo;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
	...
	private ResourceLocation exit = AssetUtils.getResource("/gui/mainmenu/exit.png");
	private ResourceLocation user = AssetUtils.getResource("/gui/mainmenu/usericon.png");
	...
	public void initGui() {
		...
		//TODO: Falcun patcher
		try {
			logo = new AnimatedLogo(Minecraft.getMinecraft().mcDataDir + "/falcunassets/falcunanimated/", 2L);
		...
		if (button.id == 4) {
			try {
				if (Desktop.isDesktopSupported()) {
					Desktop.getDesktop().browse(new URI("https://falcun.net/"));
				}
			} catch (Exception e) {
			}
		}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		String s = "Falcun Client v" + Wrapper.getInstance().getVersion();
		...
		if (!playing) {
			if (logo.getCurrentFrame() > 50) {
				logo.setPlaying(false);
				logo.setFrame(logo.getMaxFrames() - 1);
			}
		} else {
		...
		Wrapper.getInstance().drawRoundedRect(startX + startX - 50, startY - startY + 5, startX + startX - 30, startY - startY + 25, 4, isOverAccountManager ? new Color(200, 200, 200, 100).getRGB() : new Color(100, 100, 100, 100).getRGB());
		Wrapper.getInstance().drawRoundedRect(startX + startX - 50 + 1, startY - startY + 5 + 1, startX + startX - 30 - 1, startY - startY + 25 - 1, 3, new Color(22, 24, 27, 80).getRGB());

		boolean isOverExit = mouseX >= startX + startX - 25 && mouseX <= startX + startX - 5 && mouseY >= startY - startY + 5 && mouseY <= startY - startY + 25;
		Wrapper.getInstance().drawRoundedRect(startX + startX - 25, startY - startY + 5, startX + startX - 5, startY - startY + 25, 4, isOverExit ? new Color(200, 200, 200, 100).getRGB() : new Color(100, 100, 100, 100).getRGB());
		Wrapper.getInstance().drawRoundedRect(startX + startX - 25 + 1, startY - startY + 5 + 1, startX + startX - 5 - 1, startY - startY + 25 - 1, 3, new Color(22, 24, 27, 80).getRGB());

		GlStateManager.popMatrix();

		GlStateManager.pushMatrix();

		Wrapper.getInstance().drawImage(exit, startX + startX - 20, 10, 10, 10);
		Wrapper.getInstance().drawImage(user, startX + startX - 45, 10, 10, 10);
...
//			this.mc.displayGuiScreen(new GuiAccountManager());
			this.mc.displayGuiScreen(new GuiAccountSelector(this));
		}
