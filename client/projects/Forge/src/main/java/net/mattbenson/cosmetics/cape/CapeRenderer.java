...
public class CapeRenderer {
	public Map<UUID, CapeData> capeDatas;
	...
	public void applyAnimation(AbstractClientPlayer playerData) {
		...
		if(capeDatas.containsKey(playerData.getUniqueID()) && capeDatas.get(playerData.getUniqueID()).getClientPlayer() == playerData) {
			data = capeDatas.get(playerData.getUniqueID());
		} else {
			capeDatas.put(playerData.getUniqueID(), data = new CapeData(playerData));
		}
		...
		for (int i = 0; i < SLAB_AMOUNT; i++) {
			...
			this.slabs[i].setRotateAngle(
					(float) (Math.cos((data.getCapeWavePhase()) * waveSpeed + waveOffset * waveFrequency)
							* magnitude));
		}
