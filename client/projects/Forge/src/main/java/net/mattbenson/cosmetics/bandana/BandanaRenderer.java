...
import net.mattbenson.cosmetics.cape.CapeData;
...
public class BandanaRenderer {
	public Map<UUID, CapeData> bandanaDatas;
	...
	public BandanaRenderer()
	{
		bandanaDatas = new HashMap<>();
	...
	public void applyAnimation(AbstractClientPlayer playerData) {
		CapeData data = null;
		
		if(bandanaDatas.containsKey(playerData.getUniqueID()) && bandanaDatas.get(playerData.getUniqueID()).getClientPlayer() == playerData) {
			data = bandanaDatas.get(playerData.getUniqueID());
		} else {
			bandanaDatas.put(playerData.getUniqueID(), data = new CapeData(playerData));
		}
		...
		for (int i = 0; i < SLAB_AMOUNT; i++) {
			...
			this.slabs[i].setRotateAngle((float) (Math.cos((data.getCapeWavePhase()) * waveSpeed + waveOffset * waveFrequency) * magnitude));
		}
