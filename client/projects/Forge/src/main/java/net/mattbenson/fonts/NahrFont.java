...
public class NahrFont
{
    ...
    private void createFont(Object font, float size)
    {
        try
        {
            if ((font instanceof Font)) {
                ...
                this.theFont = Font.createFont(Font.TRUETYPE_FONT, (InputStream)font)
                        .deriveFont(size);
        ...
        for (int i = this.startChar; i < this.endChar; i++)
        {
            ...
            if (x >= 250 - this.theMetrics.getMaxAdvance())
            {
                ...
                y = y + (this.theMetrics.getMaxAscent() + this.theMetrics.getMaxDescent() + this.fontSize /
                        2.0F);
            }
        }
    ...
    private void drawer(String text, float x, float y, int color)
    {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if ((text.charAt(i) == cD) && (i + 1 < text.length()))
            {
                ...
                int colorCode = "0123456789abcdefklmnorg"
                        .indexOf(oneMore);
                if (colorCode < 16) {
                    try
                    {
    ...
    private void drawTexturedModalRect(double x, double y, double u, double v, double width, double height)
    {
        ...
        worldRenderer.pos(x + 0.0F, y + height, 0.0D).tex((u + 0.0F) *
                0.0039063F, (v + height) * 0.0039063F).endVertex();
        
        
 
        worldRenderer.pos(x + width, y + height, 0.0D).tex((u + width) *
                0.0039063F, (v + height) * 0.0039063F).endVertex();
        
      
        worldRenderer.pos(x + width, y + 0.0F, 0.0D).tex((u + width) *
                0.0039063F, (v + 0.0F) * 0.0039063F).endVertex();
        
        
       
        worldRenderer.pos(x + 0.0F, y + 0.0F, 0.0D).tex((u + 0.0F) *
                0.0039063F, (v + 0.0F) * 0.0039063F).endVertex();
        tesselator.draw();
    }
