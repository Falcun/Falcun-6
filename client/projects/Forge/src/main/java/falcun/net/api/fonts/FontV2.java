...
public final class FontV2 implements FalcunFont {
	...
	private void init(Font font, boolean normal) {
		this.normal = normal;
		this.characterData = new CD[LAST_CHARACTER - FIRST_CHARACTER];
		...
		for (int i = FIRST_CHARACTER; i < LAST_CHARACTER; i++) {
			...
			characterData[index] = new CD(x, y, w, h);
		...
		for (int i = FIRST_CHARACTER; i < LAST_CHARACTER; i++) {
			...
			CD charData = characterData[index];
	...
	public void drawStringFull(String string, int x, int y, int color, boolean underline) {
		if (!initd) {
			return;
		}
...
//		GlStateManager.setBlendState(oldBlendState);
		...
		if (underline) {
			int w = xPos - x;
			--y;
			Gui.drawRect(x, y + height, x + w - 4, y + height + 1, color);
		}
	...
	public void drawCenteredString(String string, int x, int y, int color) {
		...
		drawStringFull(string, x - (getStringWidthInt(string) / 2), y, color, false);
	}

	@Override
	public void drawString(String text, Number x, Number y, int color, boolean underline) {
		drawStringFull(text, x.intValue(), y.intValue(), color, underline);
	}

	public Number getStringWidth(String string) {
		return getStringWidthInt(string);
	}
