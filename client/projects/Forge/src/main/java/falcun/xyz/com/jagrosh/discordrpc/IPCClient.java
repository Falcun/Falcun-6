 ...
 */
package falcun.xyz.com.jagrosh.discordrpc;

import falcun.xyz.com.jagrosh.discordrpc.entities.*;
import falcun.xyz.com.jagrosh.discordrpc.entities.pipe.Pipe;
import falcun.xyz.com.jagrosh.discordrpc.entities.pipe.PipeStatus;
import falcun.xyz.com.jagrosh.discordrpc.exceptions.NoDiscordClientException;
import falcun.xyz.com.jagrosh.discordrpc.entities.Packet.OpCode;
...
public final class IPCClient implements Closeable {
	...
	public void sendRichPresence(RichPresence presence, Callback callback) {
		...
		pipe.send(OpCode.FRAME, new JSONObject().put("cmd", "SET_ACTIVITY").put("args", new JSONObject().put("pid", getPID()).put("activity", presence == null ? null : presence.toJson())), callback);
	}
		...
		readThread = new Thread(() -> {
			try {
				...
				while ((p = pipe.read()).getOp() != OpCode.CLOSE) {
					...
					switch (event) {
						case NULL:
							if (nonce != null && callbacks.containsKey(nonce)) callbacks.remove(nonce).succeed(p);
							break;
				...
				if (listener != null) listener.onClose(this, p.getJson());
			} catch (IOException | JSONException ex) {
				if (ex instanceof IOException) LOGGER.error("Reading thread encountered an IOException", ex);
				else LOGGER.error("Reading thread encountered an JSONException", ex);

				pipe.setStatus(PipeStatus.DISCONNECTED);
				if (listener != null) listener.onDisconnect(this, ex);
			}
		});
