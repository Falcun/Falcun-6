...
import falcun.xyz.accswitcher.ru.vidtu.ias.account.MojangAccount;
import falcun.xyz.accswitcher.ru.vidtu.ias.account.OfflineAccount;
import falcun.xyz.accswitcher.ru.vidtu.iasfork.msauth.MicrosoftAccount;
import net.minecraft.client.Minecraft;
import falcun.xyz.accswitcher.ru.vidtu.ias.Config;
import falcun.xyz.accswitcher.the_fireplace.ias.IAS;
import falcun.xyz.accswitcher.the_fireplace.ias.account.ExtendedAccountData;
...
public class Converter {
		...
		try { //Convert shared IASX.
			...
			if (iasx.exists()) {
				...
				try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(iasx))) {
					falcun.xyz.accswitcher.com.github.mrebhan.ingameaccountswitcher.tools.Config o = (falcun.xyz.accswitcher.com.github.mrebhan.ingameaccountswitcher.tools.Config) ois.readObject();
		...
		try { //Convert bugged v1 (singleton) Microsoft account storage using shared pass.
			...
			if (iasms.exists()) {
				...
				try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(iasms))) {
					Config.accounts.add(convertMicrosoft(iasp, (MicrosoftAccount) ois.readObject()));
				}
				Config.save(mc);
				iasms.delete();
	 ...
	 */
	public static falcun.xyz.accswitcher.ru.vidtu.ias.account.MicrosoftAccount convertMicrosoft(File pwdfile, MicrosoftAccount oma) throws Throwable {
		falcun.xyz.accswitcher.ru.vidtu.ias.account.MicrosoftAccount ma;
		if (pwdfile != null && pwdfile.isFile()) {
			//Name is not encrypted.
			ma = new falcun.xyz.accswitcher.ru.vidtu.ias.account.MicrosoftAccount(oma.username, decrypt(pwdfile, oma.accessToken), decrypt(pwdfile, oma.refreshToken), null); //UUID is null, but it will be got on first login.
		} else {
			ma = new falcun.xyz.accswitcher.ru.vidtu.ias.account.MicrosoftAccount(oma.username, oma.accessToken, oma.refreshToken, null); //UUID is null, but it will be got on first login.
		}
		ma.syncRefresh();
		return ma;
	}
