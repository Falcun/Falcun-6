...
public class BlockList {
    public List<WrappedItemStack> getList(final EntityPlayer player, final SchematicWorld world, final World mcWorld) {
        ...
        for (final MBlockPos pos : BlockPosHelper.getAllInBox(BlockPos.ORIGIN, new BlockPos(world.getWidth() - 1, world.getHeight() - 1, world.getLength() - 1))) {
            ...
            final WrappedItemStack wrappedItemStack = findOrCreateWrappedItemStackFor(blockList, stack, mcPos);
            if (isPlaced) {
                wrappedItemStack.placed++;
            } else {
            	wrappedItemStack.positions.add(new BlockPos(mcPos.getX(),mcPos.getY(),mcPos.getZ()));
            }
    ...
    private WrappedItemStack findOrCreateWrappedItemStackFor(final List<WrappedItemStack> blockList, final ItemStack itemStack, MBlockPos pos) {
