...
public abstract class SchematicFormat {
    ...
    public static ISchematic readFromFile(final File directory, final String filename) {
      ...
      final Collection<File> filesSchematics = FileUtils.listFiles(directory, extenstions, true);
      for (final File file : filesSchematics) {
     	 if (file.getName().equals(filename)) {
     	 name = file.getName();
     	 directory1 = file.getParent();
     	 }
      }
        } catch (final Exception ex) {
      ...
      return readFromFile(new File(directory1, name)); 
	...
	public static File readFile(File directory, String filename) {
      ...
      return new File(directory1, name); 
	}
}
