...
public class Texture {
	...
	public Texture(int w, int h, int fillColour, int minFilter, int maxFilter,
			int textureWrap) {
		...
		GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGBA8, w, h, 0,
				GL12.GL_BGRA, GL11.GL_UNSIGNED_BYTE, this.pixelBuf);
		this.setTexParameters(minFilter, maxFilter, textureWrap);
	}

	public Texture(int w, int h, int fillColour) {
		this(w, h, fillColour, GL11.GL_LINEAR, GL11.GL_NEAREST,
	...
	public Texture(int id) {
		...
		Logging.log(
				"created new MwTexture from GL texture id %d (%dx%d) (%d pixels)",
				this.id, this.w, this.h, this.pixelBuf.limit());
	}
	...
	public synchronized void close() {
		if (this.id != 0) {
			try {
				...
				Logging.log(
						"MwTexture.close: null pointer exception (texture %d)",
						this.id);
			}
			this.id = 0;
		}
	...
	public synchronized void getRGB(int x, int y, int w, int h, int[] pixels,
			int offset, int scanSize, TextureAtlasSprite icon) {
		...
		for (int i = 0; i < h; i++) {
			try {
				...
				Logging.logWarning(
						"MwTexture.getRGB: IllegalArgumentException (icon name: %s; height: %d; width: %d; MaxU: %f; MinU: %f; MaxV: %f; MinV: %f)",
						icon.getIconName(), icon.getIconHeight(),
						icon.getIconWidth(), icon.getMaxU(), icon.getMinU(),
						icon.getMaxV(), icon.getMinV());
				Logging.logWarning(
						"MwTexture.getRGB: IllegalArgumentException (pos: %d)",
						bufOffset + (i * this.w));
				Logging.logWarning(
						"MwTexture.getRGB: IllegalArgumentException (buffersize: %d)",
						this.pixelBuf.limit());
			}
	...
	public synchronized void setRGB(int x, int y, int w, int h, int[] pixels,
	...
	public synchronized void updateTextureArea(int x, int y, int w, int h) {
		try {
			...
			Logging.log(
					"MwTexture.updatePixels: null pointer exception (texture %d)",
					this.id);
		}
	}
	...
	public synchronized void updateTexture() {
		this.bind();
		this.pixelBuf.position(0);
		GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGBA8, this.w, this.h,
				0, GL12.GL_BGRA, GL11.GL_UNSIGNED_BYTE, this.pixelBuf);
	}
	...
	private synchronized void getPixelsFromExistingTexture() {
		try {
			...
			GL11.glGetTexImage(GL11.GL_TEXTURE_2D, 0, GL12.GL_BGRA,
					GL11.GL_UNSIGNED_BYTE, this.pixelBuf);
			...
			Logging.log(
					"MwTexture.getPixels: null pointer exception (texture %d)",
					this.id);
		}
	}
}
