...
public class Render {
	...
	public static void setColourWithAlphaPercent(int colour, int alphaPercent) {
		setColour(((((alphaPercent * 0xff) / 100) & 0xff) << 24)
				| (colour & 0xffffff));
	}
	...
	public static int getAverageOfPixelQuad(int[] pixels, int offset,
			int scanSize) {
		...
		int r = ((p00 >> 16) & 0xff) + ((p01 >> 16) & 0xff)
				+ ((p10 >> 16) & 0xff) + ((p11 >> 16) & 0xff);
		r >>= 2;
		int g = ((p00 >> 8) & 0xff) + ((p01 >> 8) & 0xff) + ((p10 >> 8) & 0xff)
				+ ((p11 >> 8) & 0xff);
		g >>= 2;
	...
	public static void printBoundTextureInfo(int texture) {
		...
		int depth = GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0,
				GL12.GL_TEXTURE_DEPTH);
		int format = GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0,
				GL11.GL_TEXTURE_INTERNAL_FORMAT);
		Logging.log(
				"texture %d parameters: width=%d, height=%d, depth=%d, format=%08x",
				texture, w, h, depth, format);
	}
	...
	public static void drawArrow(double x, double y, double angle, double length) {
		...
		renderer.pos(x + (length * Math.cos(angle)),
				y + (length * Math.sin(angle)), zDepth).endVertex();
		renderer.pos(x + (length * 0.5D * Math.cos(angle - arrowBackAngle)),
				y + (length * 0.5D * Math.sin(angle - arrowBackAngle)), zDepth)
				.endVertex();
		renderer.pos(x + (length * 0.3D * Math.cos(angle + Math.PI)),
				y + (length * 0.3D * Math.sin(angle + Math.PI)), zDepth)
				.endVertex();
		renderer.pos(x + (length * 0.5D * Math.cos(angle + arrowBackAngle)),
				y + (length * 0.5D * Math.sin(angle + arrowBackAngle)), zDepth)
				.endVertex();
	...
	public static void drawTriangle(double x1, double y1, double x2, double y2,
			double x3, double y3) {
		GlStateManager.enableBlend();
		GlStateManager.disableTexture2D();
		GlStateManager
	...
	public static void drawCircle(double x, double y, double r) {
		...
		for (double theta = -incr; theta < end; theta += incr) {
			renderer.pos(x + (r * Math.cos(-theta)),
					y + (r * Math.sin(-theta)), zDepth).endVertex();
		}
		// renderer.finishDrawing();
		tessellator.draw();
		GlStateManager.enableTexture2D();
	...
	public static void drawCircleBorder(double x, double y, double r,
			double width) {
		GlStateManager.enableBlend();
		GlStateManager.disableTexture2D();
		GlStateManager
		...
		for (double theta = -incr; theta < end; theta += incr) {
			...
			renderer.pos(x + (r2 * Math.cos(-theta)),
					y + (r2 * Math.sin(-theta)), zDepth).endVertex();
		}
		// renderer.finishDrawing();
		tessellator.draw();
		GlStateManager.enableTexture2D();
	...
	public static void drawRectBorder(double x, double y, double w, double h,
			double bw) {
		// top border
	...
	public static void drawString(int x, int y, int colour,
	...
	public static void drawCentredString(int x, int y, int colour,
			String formatString, Object... args) {
		Minecraft mc = Minecraft.getMinecraft();
		// mc.renderEngine.resetBoundTexture();
		FontRenderer fr = mc.fontRendererObj;
