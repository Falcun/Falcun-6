...
public class MarkerManagerGroup
{

	public List<Marker> markerList = new CopyOnWriteArrayList<Marker>();
	public List<String> groupList = new CopyOnWriteArrayList<String>();

	public List<Marker> visibleMarkerList = new CopyOnWriteArrayList<Marker>();

	private String visibleGroupName = "all";

	public Marker selectedMarker = null;

	public MarkerManagerGroup()
	{
	}
	...
	{
		return String.format("%s:%d:%d:%d:%d:%06x:%s:%s", marker.name, marker.x, marker.y, marker.z, marker.dimension, marker.colour & 0xffffff, marker.groupName, marker.visible, marker.server);
	}
	...
	public Marker stringToMarker(String s)
	{
		...
		if (split.length == 9)
		{
			try
			{
				...
				boolean visible = Boolean.parseBoolean(split[7]);
				String server = split[8];
				marker = new Marker(split[0], split[6], x, y, z, dimension, colour, server);
	...
	public void addMarker(String name, String groupName, int x, int y, int z, int dimension, int colour, String server)
	{
		this.addMarker(new Marker(name, groupName, x, y, z, dimension, colour, server));
	}
	...
	public void drawMarkers(MapMode mapMode, MapView mapView)
	{
		for (Marker marker : this.visibleMarkerList)
		{
			...
			if (mapView.getDimension() == marker.dimension && marker.visible)
			{
	...
	public void drawMarkersWorld(float partialTicks)
	{
		...
		{
			if (m.dimension == Minecraft.getMinecraft().thePlayer.dimension && m.visible && m.server.equals(NetworkUtils.getServer()))
			{
	...
	public void drawLabel(Marker m)
	{
		...
		worldrenderer.pos(strTextWidth + 1, (8), 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strTextWidth + 1, (-1), 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(-strGroupWidth - 1, (-1), 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(-strGroupWidth - 1, (8), 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strGroupWidth + 1, (8), 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
		worldrenderer.pos(strGroupWidth + 1, (-1), 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
		tessellator.draw();

		if(Config.drawMarkersDistanceInWorld) {
			worldrenderer.begin(GL11.GL_QUADS, DefaultVertexFormats.POSITION_COLOR);
			...
			worldrenderer.pos(strDistanceWidth + 1, 8 + offstet, 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
			worldrenderer.pos(strDistanceWidth + 1, -1 + offstet, 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
			worldrenderer.pos(-strGroupWidth - 1, -1 + offstet, 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
			worldrenderer.pos(-strGroupWidth - 1, 8 + offstet, 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
			worldrenderer.pos(strGroupWidth + 1, 8 + offstet, 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
			worldrenderer.pos(strGroupWidth + 1, -1 + offstet, 0.0D).color(fRed, fGreen, fBlue, fAlpha).endVertex();
			tessellator.draw();
		}
		...
		Fonts.ubuntuFont.drawString(strText, -strTextWidth, 0, -1);

		//fontrenderer.drawString(strText, -strTextWidth, 0, -1);
		Fonts.ubuntuFont.drawString(strGroup, -strGroupWidth, -10, m.colour);
		if(Config.drawMarkersDistanceInWorld) {
			Fonts.ubuntuFont.drawString(strDistance, -strDistanceWidth, offstet, -1);
		}
