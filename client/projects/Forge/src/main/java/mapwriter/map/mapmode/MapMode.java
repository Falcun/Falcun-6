...
public class MapMode {
	...
	public void setScreenRes(int dw, int dh, int sw, int sh, double scaling) {
		if ((sw != this.sw) || (sh != this.sh)
				|| (scaling != this.screenScalingFactor)) {
			this.sw = sw;
			this.sh = sh;
	...
	public void setScreenRes() {
		...
		this.setScreenRes(mc.displayWidth, mc.displayHeight,
				sRes.getScaledWidth(), sRes.getScaledHeight(),
				sRes.getScaleFactor());
	}
	...
	public void updateMargin() {
		...
		if (this.config.Position
				.equals(MapModeConfig.miniMapPositionStringArray[0])) {
			this.marginTop = 10;
			this.marginBottom = -1;
			this.marginLeft = -1;
			this.marginRight = 10;
		...
		// top left
				...
				.equals(MapModeConfig.miniMapPositionStringArray[1])) {
			this.marginTop = 10;
			this.marginBottom = -1;
			this.marginLeft = 10;
			this.marginRight = -1;
		...
		// botom right
				...
				.equals(MapModeConfig.miniMapPositionStringArray[2])) {
			this.marginTop = -1;
		...
		// botom left
				...
				.equals(MapModeConfig.miniMapPositionStringArray[3])) {
			this.marginTop = -1;
			this.marginBottom = 40;
			this.marginLeft = 10;
			this.marginRight = -1;
	...
	public Point screenXYtoBlockXZ(MapView mapView, int sx, int sy) {
		double withinMapX = ((double) (sx - this.xTranslation))
				/ ((double) this.w);
		double withinMapY = ((double) (sy - this.yTranslation))
				/ ((double) this.h);
		int bx = (int) Math.floor((mapView.getX() + (withinMapX * mapView
				.getWidth())));
		int bz = (int) Math.floor((mapView.getZ() + (withinMapY * mapView
				.getHeight())));
