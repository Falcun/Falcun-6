...
public class Trail {
	...
	class TrailMarker {
		...
		public void draw(MapMode mapMode, MapView mapView) {
			if (mapView.isBlockWithinView(this.x, this.z,
					mapMode.config.circular)) {
				Point.Double p = mapMode.blockXZtoScreenXY(mapView, this.x,
						this.z);

				// draw a coloured arrow centered on the calculated (x, y)
				Render.setColourWithAlphaPercent(borderColour,
						this.alphaPercent);
				Render.drawArrow(p.x, p.y, this.heading,
						mapMode.config.trailMarkerSize);
				Render.setColourWithAlphaPercent(colour, this.alphaPercent);
				Render.drawArrow(p.x, p.y, this.heading,
						mapMode.config.trailMarkerSize - 1.0);
			}
		}
	}
	...
	public Trail(Mw mw, String name) {
		...
		this.maxLength = ConfigurationHandler.configuration.getInt(this.name
				+ "TrailMaxLength", Reference.catOptions, this.maxLength, 1,
				200, "");
		this.intervalMillis = ConfigurationHandler.configuration.getInt(
				this.name + "TrailMarkerIntervalMillis", Reference.catOptions,
				(int) this.intervalMillis, 100, 360000, "");
	}
	...
	public void onTick() {
		...
		if ((time - this.lastMarkerTime) > this.intervalMillis) {
			...
			this.addMarker(this.mw.playerX, this.mw.playerY, this.mw.playerZ,
