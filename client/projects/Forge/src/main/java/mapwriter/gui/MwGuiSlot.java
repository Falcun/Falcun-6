...
public abstract class MwGuiSlot {
	...
	public MwGuiSlot(Minecraft mcIn, int width, int height, int topIn,
			int bottomIn, int left) {
		this.mc = mcIn;
		this.width = width;
		this.height = height;
		this.top = topIn;
	...
	public void setDimensions(int widthIn, int heightIn, int topIn,
			int bottomIn, int left) {
		this.width = widthIn;
			...
			if (Mouse.isButtonDown(0) && this.getEnabled()) {
				if (this.initialClickY == -1.0F) {
					...
					if ((this.mouseY >= this.top) && (this.mouseY <= this.bottom)) {
						...
						int l = this.getSlotIndexFromScreenCoords(this.mouseX,
								this.mouseY);
	...
	protected void drawSelectionBox(int x, int y, int mouseXIn, int mouseYIn) {
		...
		for (int index = 0; index < i1; ++index) {
			...
			if (this.showSelectionBox && this.isSelected(index)) {
				...
				worldrenderer.begin(GL11.GL_QUADS,
						DefaultVertexFormats.POSITION_TEX_COLOR);
				worldrenderer.pos(xLeft, yTotal + slotHeight, 0.0D)
						.tex(0.0D, 1.0D).color(128, 128, 128, 255).endVertex();
				worldrenderer.pos(xRight, yTotal + slotHeight, 0.0D)
						.tex(1.0D, 1.0D).color(128, 128, 128, 255).endVertex();
				worldrenderer.pos(xRight, (yTotal), 0.0D).tex(1.0D, 0.0D)
						.color(128, 128, 128, 255).endVertex();
				worldrenderer.pos(xLeft, (yTotal), 0.0D).tex(0.0D, 0.0D)
						.color(128, 128, 128, 255).endVertex();
				worldrenderer.pos(xLeft + 1, (yTotal + slotHeight) - 1, 0.0D)
						.tex(0.0D, 1.0D).color(0, 0, 0, 255).endVertex();
				worldrenderer.pos(xRight - 1, (yTotal + slotHeight) - 1, 0.0D)
						.tex(1.0D, 1.0D).color(0, 0, 0, 255).endVertex();
				worldrenderer.pos(xRight - 1, yTotal + 1, 0.0D).tex(1.0D, 0.0D)
						.color(0, 0, 0, 255).endVertex();
				worldrenderer.pos(xLeft + 1, yTotal + 1, 0.0D).tex(0.0D, 0.0D)
						.color(0, 0, 0, 255).endVertex();
				tessellator.draw();
				GlStateManager.enableTexture2D();
			}
