...
public class MwGui extends GuiScreen {
	...
	private GuiButton groupList;
	private static int groupIndex = -1;
	private GuiTextField nameField;

	private GuiButton colorButton;
	private static int colorIndex = 0;
	private String[] colors = new String[] {"RED", "YELLOW", "GRAY", "GREEN", "BLUE", "PINK"};
	...
	private String[] HelpText2 = new String[] {
		...
		, "mw.gui.mwgui.helptext.factionmap"
		, "mw.gui.mwgui.helptext.forceregenerate"
	};
	...
	public void initGui() {
		...
		if(MwAPI.getCurrentDataProvider() instanceof OverlayGroup) {
			int x = new ScaledResolution(mc).getScaledWidth() - 152;
			this.buttonList.add(clear = new GuiButton(-1, x, 23, 150, 15, "CLEAR SERVER"));

			GroupData group = GroupList.getGroupByName(Mw.getInstance().group);
			
			if(group != null) {
				clear.enabled = group.getOwner().equals(mc.thePlayer.getUniqueID());
				
				if(!clear.enabled) {
					clear.xPosition = Integer.MAX_VALUE - 500;
				} else {
					clear.xPosition = x;
				}
			}
			
			this.buttonList.add(type = new GuiButton(0, x, 57, 150, 15, ""));
			this.buttonList.add(groupList = new GuiButton(1, x, 40, 150, 15, ""));
			
			if(groupIndex == -1) {
				groupList.displayString = "Click to change group";
			} else {
				groupList.displayString = Mw.getInstance().group;
			}
			
			if(Mw.getInstance().type == 0) {
				this.buttonList.add(colorButton = new GuiButton(2, x + 50, 74, 50, 15, ""));
				colorButton.displayString = colors[colorIndex];
				type.displayString = "COLOR";
			} else if(Mw.getInstance().type == 1) {
				type.displayString = "TEXT";
				nameField = new GuiTextField(1, mc.fontRendererObj, x + 1, 75, 148, 15);
				Mw.getInstance().lastText = "";
				nameField.setText(Mw.getInstance().lastText);
			}
	...
	protected void actionPerformed(GuiButton button) {
		...
		if(button.id == -1) { //Clear
			GroupData group = GroupList.getGroupByName(Mw.getInstance().group);
			
			if(group != null) {
				NetworkingClient.sendLine("ClearChunkDatasServer", group.getId() + "");
			}
		} else if(button.id == 0) { // Type
			if(typeIndex == 0) {
				typeIndex = 1;
			} else {
				typeIndex = 0;
			...
			type.displayString = name;
			Mw.getInstance().type = typeIndex;
			initGui();
		} else if(button.id == 1) { // Group list
			if(GroupList.getGroups().isEmpty()) {
				return;
			}
			
			if(groupIndex + 1 >= GroupList.getGroups().size()) {
				groupIndex = 0;
			} else {
				groupIndex++;
			}
			
			GroupData data = GroupList.getGroups().get(groupIndex);
			
			if(data == null) {
				return;
			}
			
			String name = GroupList.getGroups().get(groupIndex).getName();
			
			groupList.displayString = name;
			Mw.getInstance().group = name;
			...
			if(colorButton.displayString.equalsIgnoreCase("RED")) {
				Mw.getInstance().color = Color.RED.getRGB();
			} else if(colorButton.displayString.equalsIgnoreCase("YELLOW")) {
				Mw.getInstance().color = Color.YELLOW.getRGB();
			} else if(colorButton.displayString.equalsIgnoreCase("GRAY")) {
				Mw.getInstance().color = Color.GRAY.getRGB();
			} else if(colorButton.displayString.equalsIgnoreCase("GREEN")) {
				Mw.getInstance().color = Color.GREEN.getRGB();
			} else if(colorButton.displayString.equalsIgnoreCase("BLUE")) {
				Mw.getInstance().color = Color.BLUE.getRGB();
			} else if(colorButton.displayString.equalsIgnoreCase("PINK")) {
				Mw.getInstance().color = Color.PINK.getRGB();
			}
	...
	protected void keyTyped(char c, int key) {
		if(nameField != null && Mw.getInstance().type == 1 && nameField.textboxKeyTyped(c, key)) {
			Mw.getInstance().lastText = nameField.getText();
			return;
		}
		...
		case Keyboard.KEY_F:
			this.mc.thePlayer.addChatMessage(new ChatComponentText("\u00A7b[MapWriter] \u00A7d Attempting to force regenerate the map."));
			this.forceRegenerate();
			this.exitGui();
			break;

		case Keyboard.KEY_V:
			this.mw.facInput.showMap = !this.mw.facInput.showMap;
			this.mc.thePlayer.addChatMessage(new ChatComponentText("\u00A7b[MapWriter] \u00A7d/f map is " + (this.mw.facInput.showMap ? "now" : "not ") + " going to show in chat."));
			break;
	...
	public void handleMouseInput() throws IOException {
		...
		int xPos = new ScaledResolution(mc).getScaledWidth() - 152;
		int yPos = 20;
		int height = 75;
		int width = 152;
		...
		if ((MwAPI.getCurrentDataProvider() != null) && MwAPI.getCurrentDataProvider().onMouseInput(this.mapView, this.mapMode)) {
			return;
		} else if (this.MarkerOverlay.isMouseInField() && (this.mouseLeftHeld == 0)) {
	...
	protected void mouseClicked(int x, int y, int button) {
		...
		if(Mouse.isButtonDown(0)) {
			if(groupLabel.posWithin(x, y)) {
				this.mw.markerManager.nextGroup(1);
				this.mw.markerManager.update();
				return;
			}
		}
		
		if(nameField != null && Mw.getInstance().type == 1) {
		...
		if (this.MarkerOverlay.isMouseInField() && (this.mouseLeftHeld == 0)) {
			...
			if (button == 0) {
				if (this.dimensionLabel.posWithin(x, y)) {
					...
					if (provider != null) {
						provider.onLeftClick(this.mapView.getDimension(), blockPoint.x, blockPoint.y, this.mapView, this);
					}
				}
				...
				IMwDataProvider provider = MwAPI.getCurrentDataProvider();
				if (provider != null) {
					provider.onRightClick(this.mapView.getDimension(), blockPoint.x, blockPoint.y, this.mapView, this);
				}
			}
	...
	public void onGuiClosed() {
		...
		if(Mw.getInstance().groupEdited.size() > 0) {
			for(long groupId : Mw.getInstance().groupEdited) {
				GroupData group = GroupList.getGroupById(groupId);
				
				if(group != null) {
					NetworkingClient.sendLine("FinishChunkEditing", group.getId() + "");
				}
			}
			
			Mw.getInstance().groupEdited.clear();
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float f) {
		...
		if(Mw.getInstance().type == 1) {
			nameField.drawTextBox();
		}

		String line = "";
		
		line = "Middle click - Delete";
		mc.fontRendererObj.drawStringWithShadow(line, xPos + width / 2 - mc.fontRendererObj.getStringWidth(line) / 2, yPos + height, Color.WHITE.getRGB());
		yPos += 10;
		line = "Right click - Draw";
		mc.fontRendererObj.drawStringWithShadow(line, xPos + width / 2 - mc.fontRendererObj.getStringWidth(line) / 2, yPos + height, Color.WHITE.getRGB());
	...
	private void drawMarkerLabel(int mouseX, int mouseY, float f) {
		...
		if (this.isPlayerNearScreenPos(mouseX, mouseY)) {
			this.markerLabel.setText(new String[] { this.mc.thePlayer.getDisplayName().getUnformattedText(), String.format("(%d, %d, %d)", this.mw.playerXInt, this.mw.playerYInt, this.mw.playerZInt) }, null);
			this.markerLabel.setCoords(mouseX + 8, mouseY);
			this.markerLabel.draw();
		}
	}
	...
	public void openMarkerGui(Marker m, int mouseX, int mouseY) {
		if ((m != null) && (this.mw.markerManager.selectedMarker == m)) {
			...
			if (group.equals("none")) group = I18n.format("mw.gui.mwgui.group.2");
			...
			String server =  NetworkUtils.getServer();
			
			if (Config.newMarkerDialog) {
				this.mc.displayGuiScreen(new MwGuiMarkerDialogNew(this, this.mw.markerManager, "", group, mx, my, mz, this.mapView.getDimension(), server));
			} else {
				this.mc.displayGuiScreen(new MwGuiMarkerDialog(this, this.mw.markerManager, "", group, mx, my, mz, this.mapView.getDimension(), server));
			}
		}
	}
}
