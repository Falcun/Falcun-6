...
public class MwAPI {
	...
	private static HashBiMap<String, IMwDataProvider> dataProviders = HashBiMap
			.create();
	...
	public static IMwDataProvider setCurrentDataProvider(
	...
	public static IMwDataProvider setPrevProvider() {
		if (currentProvider != null) {
			...
			if (providerKeys.size() > 0) {
				currentProvider = getDataProvider(providerKeys.get(providerKeys
						.size() - 1));
			}
		}
		return currentProvider;
