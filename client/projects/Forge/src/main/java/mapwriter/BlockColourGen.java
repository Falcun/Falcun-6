...
public class BlockColourGen {
	...
	private static int getIconMapColour(TextureAtlasSprite icon,
		...
		int iconX = Math.round((terrainTexture.w)
				* Math.min(icon.getMinU(), icon.getMaxU()));
		int iconY = Math.round((terrainTexture.h)
				* Math.min(icon.getMinV(), icon.getMaxV()));
		int iconWidth = Math.round((terrainTexture.w)
				* Math.abs(icon.getMaxU() - icon.getMinU()));
		int iconHeight = Math.round((terrainTexture.h)
				...
				if (block != null && block.getRenderType() != -1) {
					...
					if (icon != null) {
						...
						if ((u1 == u1Last) && (u2 == u2Last) && (v1 == v1Last)
							...
							// blocks on the map
							if (((ResourceLocation) Block.blockRegistry
									.getNameForObject(block))
									.getResourceDomain().contains(
											"CarpentersBlocks")) {
