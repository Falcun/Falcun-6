...
public class ChunkManager {
	...
	public static MwChunk copyToMwChunk(Chunk chunk) {
		...
		TileEntityMap = Utils.checkedMapByCopy(chunk.getTileEntityMap(),
				BlockPos.class, TileEntity.class, false);
		...
		if (storageArrays != null) {
			for (ExtendedBlockStorage storage : storageArrays) {
				if (storage != null) {
					...
					lightingArray[y] = (storage.getBlocklightArray() != null) ? Arrays
							.copyOf(storage.getBlocklightArray().getData(),
									storage.getBlocklightArray().getData().length)
							: null;
				}
			}
		}
		...
		return new MwChunk(chunk.xPosition, chunk.zPosition,
				...
				lightingArray, Arrays.copyOf(chunk.getBiomeArray(),
						chunk.getBiomeArray().length), TileEntityMap);
	}
	...
	public void updateSurfaceChunks() {
		int chunksToUpdate = Math.min(this.chunkMap.size(),
		...
		for (int i = 0; i < chunksToUpdate; i++) {
			...
			if (entry != null) {
				...
				int flags = entry.getValue();
				if (Utils.distToChunkSq(this.mw.playerXInt, this.mw.playerZInt,
						chunk) <= Config.maxChunkSaveDistSq) {
				...
				if ((flags & VISIBLE_FLAG) != 0) {
					...
					this.mw.executor.addTask(new UpdateSurfaceChunksTask(
							this.mw, chunkArray[i]));
				} else {
					chunkArray[i] = null;
				}
			}
	...
	private void addSaveChunkTask(Chunk chunk) {
		if ((Minecraft.getMinecraft().isSingleplayer() && Config.regionFileOutputEnabledMP)
			...
			if (!chunk.isEmpty()) {
				this.mw.executor.addTask(new SaveChunkTask(
						copyToMwChunk(chunk), this.mw.regionManager));
			}
		}
	}
}