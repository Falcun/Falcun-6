...
public class Region {
	...
	public Region(RegionManager regionManager, int x, int z, int zoomLevel,
			int dimension) {

		this.regionManager = regionManager;

		this.zoomLevel = Math
				.min(Math.max(0, zoomLevel), regionManager.maxZoom);
	...
	public String toString() {
		return String.format("(%d,%d) z%d dim%d", this.x, this.z,
				this.zoomLevel, this.dimension);
	}
	...
	public File getImageFile() {
		File dimDir = addDimensionDirToPath(this.regionManager.imageDir,
				this.dimension);
	...
	public boolean equals(int x, int z, int zoomLevel, int dimension) {
		x &= -this.size;
		z &= -this.size;
		return (this.x == x) && (this.z == z) && (this.zoomLevel == zoomLevel)
				&& (this.dimension == dimension);
	}
	...
	public boolean equals(Object o) {
		...
		if (o != null) {
			if (o instanceof Region) {
				...
				equal = this.equals(region.x, region.z, region.zoomLevel,
						region.dimension);
			}
		}
		return equal;
	...
	public static Long getKey(int x, int z, int zoomLevel, int dimension) {
		...
		return Long.valueOf((((long) dimension) << 40)
				| (((long) zoomLevel) << 32) | (((long) z) << 16) | (x));
	}
	...
	public Region updateNextZoomLevel(int x, int z, int w, int h) {
		...
		if (srcPixels != null) {
			...
			if (dstZoomLevel <= this.regionManager.maxZoom) {
				dstRegion = this.regionManager.getRegion(x, z, dstZoomLevel,
						this.dimension);
				...
				dstRegion.surfacePixels.updateScaled(srcPixels, srcX, srcZ,
						dstX, dstZ, dstW, dstH);
			}
		}
