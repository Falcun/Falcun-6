...
public class RegionManager {
	...
	public RegionManager(File worldDir, File imageDir,
			BlockColours blockColours, int minZoom, int maxZoom) {
	...
	public Region getRegion(int x, int z, int zoomLevel, int dimension) {
		Region region = this.regionMap.get(Region.getKey(x, z, zoomLevel,
				dimension));
		if (region == null) {
			// add region
	...
	public void updateChunk(MwChunk chunk) {
		Region region = this.getRegion(chunk.x << 4, chunk.z << 4, 0,
				chunk.dimension);
		region.updateChunk(chunk);
	}

	public void rebuildRegions(int xStart, int zStart, int w, int h,
			int dimension) {
		...
		Logging.logInfo("rebuilding regions from (%d, %d) to (%d, %d)", xStart,
				zStart, xStart + w, zStart + h);
				...
				if (this.regionFileCache.regionFileExists(rX, rZ, dimension)) {
					...
					for (int cz = 0; cz < 32; cz++) {
						for (int cx = 0; cx < 32; cx++) {
							...
							MwChunk chunk = MwChunk.read((region.x >> 4) + cx,
									(region.z >> 4) + cz, region.dimension,
									this.regionFileCache);
							region.updateChunk(chunk);
						}
					}
				}
